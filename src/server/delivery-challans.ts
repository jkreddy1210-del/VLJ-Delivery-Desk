...
import { buildChallanNumber, getFinancialYearLabel, isDeliveryType, suffixForDeliveryType, type DeliveryTypeValue, type MovementReasonValue } from "@/lib/challan-number";
...
const normalizeChallanItemInput = (data: { stockItemId: number; quantity: number | string; rate: number | string; amount?: number | string; gstRate?: number | string; remarks?: string }) => {
  const quantity = Number(data.quantity ?? 0); const rate = Number(data.rate ?? 0); const amount = Number((Number(data.amount ?? quantity * rate)).toFixed(2)); const gstRate = Number(data.gstRate ?? 3);
...
export type ChallanInput = {
  challanNumber?: string; challanDate?: string | Date; deliveryType?: DeliveryTypeValue; direction?: MovementDirectionValue; movementReason?: MovementReasonValue; againstVoucherNo?: string; invoiceNo?: string; noteType?: "CREDIT" | "DEBIT" | null; noteNo?: string; roundoff?: number | string; customerId?: number; transporterId?: number | null; placeOfSupply?: string; referenceNo?: string; referenceDate?: string | Date | null; buyerOrderNo?: string; dispatchDocNo?: string; modeOfPayment?: string; otherReferences?: string; destination?: string; termsOfDelivery?: string; remarks?: string; status?: ChallanStatusValue;
};
const normalizeChallanInput = (data: ChallanInput) => ({
  ...
  againstVoucherNo: optionalText(data.againstVoucherNo), invoiceNo: optionalText(data.invoiceNo), noteType: data.noteType ?? null, noteNo: optionalText(data.noteNo), roundoff: toDecimal(Number(data.roundoff ?? 0)), customerId: Number(data.customerId ?? 0), transporterId: data.transporterId ? Number(data.transporterId) : null,
  placeOfSupply: optionalText(data.placeOfSupply), referenceNo: optionalText(data.referenceNo), referenceDate: data.referenceDate ? new Date(data.referenceDate) : null, buyerOrderNo: optionalText(data.buyerOrderNo), dispatchDocNo: optionalText(data.dispatchDocNo), modeOfPayment: optionalText(data.modeOfPayment), otherReferences: optionalText(data.otherReferences), destination: optionalText(data.destination), termsOfDelivery: optionalText(data.termsOfDelivery), remarks: optionalText(data.remarks), status: data.status ?? "STOCK_SENT",
});
const challanInclude = { customer: true, transporter: true, items: { include: { stockItem: true } } } as const;

function counterField(deliveryType: DeliveryTypeValue) {
  if (deliveryType === "JOB_WORK") return "challanNextNoJobWork" as const;
  if (deliveryType === "MARKETING") return "challanNextNoMarketing" as const;
  return "challanNextNoApproval" as const;
}
function suffixField(deliveryType: DeliveryTypeValue) {
  if (deliveryType === "JOB_WORK") return "challanSuffixJobWork" as const;
  if (deliveryType === "MARKETING") return "challanSuffixMarketing" as const;
  return "challanSuffixApproval" as const;
}
...
export async function listDeliveryChallans({ search, status = "ALL", deliveryType = "ALL", direction = "ALL", fromDate, toDate, page = 1, pageSize = 10 }: { search?: string; status?: "ALL" | ChallanStatusValue; deliveryType?: "ALL" | DeliveryTypeValue; direction?: "ALL" | MovementDirectionValue; fromDate?: string; toDate?: string; page?: number; pageSize?: number }) {
  const take = Math.min(Math.max(pageSize, 1), 500); const skip = (Math.max(page, 1) - 1) * take; const searchTerm = search?.trim(); const challanDate: { gte?: Date; lte?: Date } = {};
  if (fromDate) challanDate.gte = new Date(fromDate); if (toDate) { const end = new Date(toDate); end.setHours(23, 59, 59, 999); challanDate.lte = end; }
...
export async function getDeliveryChallan(id: number) { return serializeChallan(await prisma.deliveryChallan.findUnique({ where: { id }, include: challanInclude })); }
...
export async function createDeliveryChallan(data: ChallanInput & { items: Array<{ stockItemId: number; quantity: number | string; rate: number | string; amount?: number | string; gstRate?: number | string; remarks?: string }> }) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    let challanNumber = normalized.challanNumber?.trim();
    if (!challanNumber) {
      const settings = await tx.companySettings.findFirst({ orderBy: { id: "asc" } });
      if (!settings) throw new Error("Company settings are not configured. Set them before creating a new voucher.");
      const type = normalized.deliveryType;
      const field = counterField(type);
      const suffix = settings[suffixField(type)] ?? suffixForDeliveryType(type);
      const nextNo = settings[field] ?? 1;
      const financialYear = settings.challanFinancialYear?.trim() || getFinancialYearLabel(normalized.challanDate);
      challanNumber = buildChallanNumber({ prefix: settings.challanPrefix, nextNo, financialYear, suffix, date: normalized.challanDate });
      await tx.companySettings.update({ where: { id: settings.id }, data: { [field]: { increment: 1 } } });
    }

    const direction = normalized.direction; const transactionType = direction === "INWARD" ? "RECEIVE" : "SEND"; const status = direction === "INWARD" ? "STOCK_RECEIVED" : "STOCK_SENT";
...
export async function deleteDeliveryChallan(id: number) {
  return prisma.$transaction(async (tx) => {
    ...
    for (const pair of customerProductPairs) {
      const [customerId, productId] = pair.split(":").map(Number); const entries = await tx.stockLedger.findMany({ where: { customerId, productId } }); let balance = new Prisma.Decimal(0);
