...
import { getNextChallanNumberFn } from "@/routes/api/company-settings";
...
export const Route = createFileRoute("/delivery-challans/new")({ head: () => ({ meta: [{ title: "New Voucher — VLJ Delivery Desk" }] }), component: NewChallan });
...
function NewChallan() {
  const navigate = useNavigate(); const location = useLocation(); const queryCustomerId = new URLSearchParams(location.search).get("customerId") ?? "";
  const [customers, setCustomers] = useState<Customer[]>([]); const [products, setProducts] = useState<Product[]>([]); const [transporters, setTransporters] = useState<Array<{ id: number; name: string }>>([]);
  const [customerId, setCustomerId] = useState(queryCustomerId); const [transporterId, setTransporterId] = useState("none"); const [challanNumber, setChallanNumber] = useState(""); const [numberMode, setNumberMode] = useState<"AUTO" | "MANUAL">("AUTO");
  const [challanDate, setChallanDate] = useState(() => new Date().toISOString().slice(0, 10)); const [deliveryType, setDeliveryType] = useState<DeliveryTypeValue | "">(""); const [direction, setDirection] = useState<"OUTWARD" | "INWARD">("OUTWARD");
  const [movementReason, setMovementReason] = useState<MovementReasonValue>("ORIGINAL"); const [againstVoucherNo, setAgainstVoucherNo] = useState(""); const [invoiceNo, setInvoiceNo] = useState(""); const [noteType, setNoteType] = useState<"NONE" | "CREDIT" | "DEBIT">("NONE"); const [noteNo, setNoteNo] = useState("");
  const [placeOfSupply, setPlaceOfSupply] = useState(""); const [destination, setDestination] = useState(""); const [referenceNo, setReferenceNo] = useState(""); const [remarks, setRemarks] = useState("");
  const [items, setItems] = useState<Row[]>([{ key: createRowKey(), stockItemId: "", quantity: "", rate: "", amount: "", gstRate: "3" }]); const [saving, setSaving] = useState(false); const [numberLoading, setNumberLoading] = useState(false); const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    Promise.all([listCustomersFn({ data: { page: 1, pageSize: 500, status: "ACTIVE" } }), listProductsFn({ data: { page: 1, pageSize: 500, status: "ACTIVE" } }), listTransportersFn({ data: { page: 1, pageSize: 500, status: "ACTIVE" } })]).then(([c, p, t]) => { setCustomers(c.rows); setProducts(p.rows); setTransporters(t.rows); }).catch(() => setError("Failed to load masters."));
  }, []);
  ...
  useEffect(() => {
    if (numberMode !== "AUTO" || !deliveryType) return;
    let cancelled = false; setNumberLoading(true);
    getNextChallanNumberFn({ data: { deliveryType, date: challanDate } }).then((next) => { if (!cancelled) setChallanNumber(String(next)); }).catch(() => { if (!cancelled) setError("Unable to generate the next voucher number. You can switch to manual numbering."); }).finally(() => { if (!cancelled) setNumberLoading(false); });
    return () => { cancelled = true; };
  }, [deliveryType, challanDate, numberMode]);
  ...
  const updateItem = (key: string, patch: Partial<Row>) => setItems((rows) => rows.map((row) => { if (row.key !== key) return row; const next = { ...row, ...patch }; if (patch.quantity !== undefined || patch.rate !== undefined) next.amount = String(Number((toNumber(next.quantity) * toNumber(next.rate)).toFixed(2)) || ""); return next; }));

  async function save() {
    setError(null); if (!customerId) return setError("Select party."); if (!deliveryType) return setError("Select delivery type.");
    const validItems = items.filter((item) => item.stockItemId && toNumber(item.quantity) > 0); if (!validItems.length) return setError("Add at least one item with quantity.");
    setSaving(true);
    try {
      await createDeliveryChallanFn({ data: { challanNumber: numberMode === "AUTO" ? undefined : challanNumber.trim(), challanDate, customerId: Number(customerId), transporterId: transporterId === "none" ? null : Number(transporterId), deliveryType, direction, movementReason, againstVoucherNo: againstVoucherNo || undefined, invoiceNo: invoiceNo || undefined, noteType: noteType === "NONE" ? null : noteType, noteNo: noteNo || undefined, placeOfSupply, destination, referenceNo, remarks, items: validItems.map((item) => ({ stockItemId: Number(item.stockItemId), quantity: toNumber(item.quantity), rate: toNumber(item.rate), amount: item.amount ? toNumber(item.amount) : undefined, gstRate: toNumber(item.gstRate) })) } });
  ...
  return <div className="space-y-6">
    ...
    <FormSection title="Voucher" description="The movement direction controls the inventory ledger.">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <Field label="Voucher No." required><div className="flex gap-2"><Input value={challanNumber} readOnly={numberMode === "AUTO"} onChange={(e) => setChallanNumber(e.target.value)} placeholder="VLJ/001/2026-27-A" />{numberMode === "AUTO" ? <Button type="button" variant="outline" size="icon" title="Refresh number" disabled={numberLoading || !deliveryType} onClick={() => setNumberLoading(false)}><RefreshCw size={15} /></Button> : null}</div><button type="button" className="mt-1 text-xs text-primary hover:underline" onClick={() => { setNumberMode((m) => m === "AUTO" ? "MANUAL" : "AUTO"); if (numberMode === "AUTO") setChallanNumber(""); }}>{numberMode === "AUTO" ? "Use manual number (for legacy/back-entry)" : "Use automatic number"}</button></Field>
    ...
    <FormSection title="Party" description="The same party can be used for both inward and outward vouchers.">
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Field label="Place of Supply"><Input value={placeOfSupply} onChange={(e) => setPlaceOfSupply(e.target.value)} /></Field><Field label="Destination"><Input value={destination} onChange={(e) => setDestination(e.target.value)} /></Field>
      </div>
    </FormSection>
    <FormSection title="Movement Reference" description="Use these fields when this voucher is a return or follows an invoice.">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <Field label="Against Voucher No."><Input value={againstVoucherNo} onChange={(e) => setAgainstVoucherNo(e.target.value)} placeholder="Original DC / voucher" /></Field><Field label="Invoice No."><Input value={invoiceNo} onChange={(e) => setInvoiceNo(e.target.value)} /></Field>
        <Field label="Credit / Debit Note"><div className="grid grid-cols-2 gap-2"><Select value={noteType} onValueChange={(v) => setNoteType(v as "NONE" | "CREDIT" | "DEBIT")}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="NONE">None</SelectItem><SelectItem value="CREDIT">Credit</SelectItem><SelectItem value="DEBIT">Debit</SelectItem></SelectContent></Select><Input value={noteNo} onChange={(e) => setNoteNo(e.target.value)} disabled={noteType === "NONE"} placeholder="No." /></div></Field><Field label="Reference No."><Input value={referenceNo} onChange={(e) => setReferenceNo(e.target.value)} /></Field>
      </div>
    </FormSection>
    <FormSection title="Description of Goods" description="Inventory is posted automatically when the voucher is saved.">
      ...
      {error ? <p className="mt-4 text-sm text-destructive">{error}</p> : null}<div className="mt-6 flex gap-3"><Button type="button" variant="outline" asChild><Link to="/delivery-challans">Cancel</Link></Button><Button type="button" onClick={save} disabled={saving || numberLoading}>{saving ? "Saving..." : "Save Voucher"}</Button></div>
    </FormSection>
  </div>;
}
