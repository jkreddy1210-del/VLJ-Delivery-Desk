...
import { listCustomersFn, deleteCustomerFn, deleteCustomerForeverFn, restoreCustomerFn } from "@/routes/api/customers";
...
export const Route = createFileRoute("/customers/")({
  head: () => ({
    meta: [
      { title: "Customers — VLJ Delivery Desk" },
      { name: "description", content: "Manage your customer and vendor master." },
      { property: "og:title", content: "Customers — VLJ Delivery Desk" },
      { property: "og:description", content: "Manage your customer and vendor master." },
    ],
  }),
...
function CustomersPage() {
  ...
  const handleDeleteForever = async (id: number) => {
    try {
      await deleteCustomerForeverFn({ data: { id } });
      setRows((prev) => prev.filter((customer) => customer.id !== id));
  ...
  const handleRestore = async (id: number) => {
    try {
      await restoreCustomerFn({ data: { id } });
      setRows((prev) => prev.filter((customer) => customer.id !== id));
      toast.success("Party restored successfully.");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Failed to restore party.");
    }
  };
  ...
  return (
    <>
      ...
      <DataPanel title="All Parties" caption={`${total} record${total === 1 ? "" : "s"}`}>
        <div className="border-b border-border px-5 py-3">
          <div className="flex items-center gap-3">
            <div className="relative max-w-sm flex-1">
              ...
              <Input value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} placeholder="Search parties..." className="h-9 pl-9" />
            </div>
        ...
        <DataTable columns={["Ledger Name", "Party Type", "GSTIN", "City", "State", "Status", "Actions"]}>
          {loading ? (
            <tr><td colSpan={7} className="px-5 py-6 text-sm text-muted-foreground">Loading parties...</td></tr>
          ) : rows.length === 0 ? (
            <tr><td colSpan={7} className="p-0"><div className="flex flex-col items-center gap-3 px-6 py-10 text-center"><Users size={20} className="text-muted-foreground" /><div><h3 className="font-display text-base font-semibold text-foreground">No parties found</h3><p className="mt-1 text-sm text-muted-foreground">Add your first customer or vendor, or import masters from Tally XML.</p></div><Button asChild size="sm"><Link to="/customers/new">Add party</Link></Button></div></td></tr>
          ) : (
            rows.map((customer) => (
