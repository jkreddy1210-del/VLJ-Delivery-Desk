...
export const Route = createFileRoute("/delivery-challans/customer/$customerName")({
  head: ({ params }) => ({
    meta: [
      { title: `${params.customerName} Vouchers — VLJ Delivery Desk` },
      {
        name: "description",
        content: `Existing delivery vouchers for ${params.customerName}.`,
      },
      { property: "og:title", content: `${params.customerName} Vouchers — VLJ Delivery Desk` },
      {
...
type DeliveryChallanRow = {
  ...
  customer?: { id?: number | null; ledgerName?: string | null } | null;
...
function CustomerDeliveryChallansPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        const result = await listDeliveryChallansFn({
  ...
  const totalQty = rows.reduce(
    (sum, row) => sum + (row.items?.reduce((itemSum, item) => itemSum + Number(item.quantity ?? 0), 0) ?? 0),
    0,
  );

  const totalValue = rows.reduce(
    (sum, row) => sum + (row.items?.reduce((itemSum, item) => itemSum + Number(item.amount ?? 0), 0) ?? 0),
    0,
  );
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-3">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h1 className="text-3xl font-semibold text-foreground">{customerName}</h1>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Button asChild>
              <Link
                to={`/delivery-challans/new${customerId ? `?customerId=${encodeURIComponent(customerId)}` : ""}`}
              >
                Add Voucher
              </Link>
            </Button>
            <Button asChild variant="outline">
              <Link to="/delivery-challans">Back to vouchers</Link>
            </Button>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <div className="relative flex-1">
            ...
            <Input
              ...
              placeholder="Search vouchers for this customer..."
              className="h-10 pl-9"
            />
          </div>
          <Select
      ...
      <section className="surface-panel overflow-hidden rounded-xl border border-border">
        <DataTable
          columns={[
            "Sl No.",
            "Voucher No",
        ...
        >
          {loading ? (
            ...
            rows.map((challan, index) => (
              <tr key={challan.id} className="border-b border-border last:border-0">
                <td className="px-5 py-3 text-muted-foreground">{index + 1}</td>
                <td className="px-5 py-3 font-medium text-foreground">{challan.challanNumber}</td>
                <td className="px-5 py-3 text-muted-foreground">
                  {deliveryTypeLabel(challan.deliveryType)}
                </td>
                <td className="px-5 py-3 text-muted-foreground">
                  {new Date(challan.challanDate).toLocaleDateString()}
                </td>
                <td className="px-5 py-3 text-right text-muted-foreground">
                  ₹ {formatMoney((challan.items ?? []).reduce((sum, item) => sum + Number(item.amount ?? 0), 0))}
                </td>
                <td className="px-5 py-3 text-muted-foreground">{statusLabel(challan.status)}</td>
                <td className="px-5 py-3 text-muted-foreground">
                  {challan.transporter?.name ?? ""}
                </td>
                <td className="px-5 py-3">
                  <div className="flex flex-wrap gap-2">
                    <Button variant="outline" size="sm" asChild>
                      <Link
                        to="/delivery-challans/edit/$id"
                        params={{ id: String(challan.id) }}
                        search={{ returnTo: `/delivery-challans/customer/${encodeURIComponent(customerName)}` }}
                      >
                        <Pencil size={14} />
                        Edit
                      </Link>
                    ...
                    <ConfirmDialog
                      ...
                      description={`Delete voucher "${challan.challanNumber}" permanently?`}
                      confirmText="Delete"
                      cancelText="Cancel"
                      onConfirm={() => handleDeleteChallan(challan.id)}
                    />
                  </div>
                </td>
              </tr>
