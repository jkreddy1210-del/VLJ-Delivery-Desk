...
function CustomerDeliveryChallansPage() {
  ...
  return <div className="space-y-6">
    <div className="grid gap-3">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm text-muted-foreground">Customer</p><h1 className="text-3xl font-semibold text-foreground">{customerName}</h1><p className="mt-1 text-sm text-muted-foreground">{rows.length} voucher{rows.length === 1 ? "s" : "s"}{totalQty > 0 ? ` · ${totalQty} qty` : ""}{totalValue > 0 ? ` · ₹${formatMoney(totalValue)}` : ""}</p></div><div className="flex flex-wrap items-center gap-2"><Button asChild><Link to={`/delivery-challans/new${customerId ? `?customerId=${encodeURIComponent(customerId)}` : ""}`}>Add Voucher</Link></Button><Button asChild variant="outline"><Link to="/delivery-challans">Back to vouchers</Link></Button></div></div>
    ...
    {customerId ? <VoucherReconciliationPanel customerId={Number(customerId)} /> : null}
