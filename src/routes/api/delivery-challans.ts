...
const settlementSchema = z.object({ challanId: z.coerce.number().int().positive(), challanItemId: z.coerce.number().int().positive(), documentType: z.enum(["INVOICE", "CREDIT_NOTE", "DEBIT_NOTE", "RETURN_DC"]), documentNo: z.string().trim().min(1), documentDate: z.string().optional().nullable(), quantity: z.coerce.number().min(0).optional(), amount: z.coerce.number().min(0).optional(), remarks: optionalText });
